<template>
  <div>
    <div class="h-16 dark:bg-dark-800">
      ...
      <div class="flex justify-center rounded-xl">
        ...
        <div v-if="bottomNavbar.isActionButtonVisible"
          ...
          <button ref="actionButton" type="button" class="text-primary-100 w-16 transform duration-300"
            :class="bottomNavbar.isActionButtonMenuVisible ? 'rotate-0' : 'rotate-45'"
            @click="bottomNavbar.isActionButtonMenuVisible = !bottomNavbar.isActionButtonMenuVisible">
            <IconPlus />
          </button>
        </div>
    ...
    <div class="w-32 mx-auto bg-transparent absolute bottom-16 mb-10 left-0 right-0 transition duration-500 ease-in-out"
      :class="bottomNavbar.isActionButtonMenuVisible ? 'opacity-100' : 'opacity-0'">
...
onClickOutside(actionButton, () => bottomNavbar.isActionButtonMenuVisible = false)
