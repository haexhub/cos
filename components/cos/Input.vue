<template>
  <div class="flex">
    ...
    <div class="relative">
      <input v-if="isPassword" :id="id" :type="safe ? 'password' : 'text'" :placeholder="placeholder" :value="modelValue"
        @input="
          $emit('update:modelValue', ($event.target as HTMLInputElement).value)"
        class="block px-2.5 pb-2 pt-3 w-full text-sm text-gray-900 dark:text-white bg-transparent rounded-l-md border border-gray-500 appearance-none focus:outline-none focus:ring-0 focus:border-primary-700 peer hover:ring-2 ring-primary-700 transition ease-in-out duration-300" />
